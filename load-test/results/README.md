...
| 지표 | before (개선 전) | round1 (인증 개선) | round2 (조회 개선) |
|---|---|---|---|
| 실패율 | **11.01%** ❌ | _ | _ |
| 전체 응답시간 p99 | **31초** ❌ | _ | _ |
| 로그인 응답시간 (중간값) | **30초** ❌ | _ | _ |
| 500 에러 건수 | 108건 | _ | _ |
| 인증 실패 (401) 건수 | 513건 | _ | _ |

### Peak (500명 동시 사용, 5분)

| 지표 | before | round1 | round2 |
|---|---|---|---|
| 실패율 | 17.0% ❌ | _ | _ |
| 전체 응답시간 p99 | 66초 ❌ | _ | _ |
| 로그인 응답시간 (중간값) | 46초 | _ | _ |
...
|---|---|---|---|
| 실패율 | 18.3% ❌ | (미측정) | _ |
| 전체 응답시간 p99 | 112초 ❌ | _ | _ |
| `GET /usage-statements` p99 | 7초 | _ | _ |
| `GET /projects?keyword=` p99 | 93초 | _ | _ |
| `GET /dashboard` p99 | 91초 | _ | _ |
