...
LOCUST_RESULT="${LOCUST_RESULT:-k8s_$(date +%Y%m%d_%H%M)}"  # 결과 CSV 파일명 (legacy — LOCUST_OUT_DIR 미설정 시 사용)
LOCUST_OUT_DIR="${LOCUST_OUT_DIR:-}"                 # 새 폴더 구조 저장 경로 (예: round2/baseline_200u). 설정 시 stats.csv 등 단순화 + meta.txt 자동
LOCUST_PHASE="${LOCUST_PHASE:-}"                     # meta.txt phase 표시 (미설정 시 LOCUST_USERS 기반 추정)
...
# 출력 경로 결정: LOCUST_OUT_DIR 설정 시 새 구조, 미설정 시 legacy(prefix) 모드
# 호출 후: OUTPUT_DIR, CSV_PREFIX, USE_OUT_DIR 변수 세팅됨
resolve_output() {
  if [ -n "${LOCUST_OUT_DIR:-}" ]; then
    OUTPUT_DIR="$SCRIPT_DIR/results/$LOCUST_OUT_DIR"
    CSV_PREFIX="$OUTPUT_DIR/_tmp"
    USE_OUT_DIR=true
  else
    OUTPUT_DIR="$SCRIPT_DIR/results"
    CSV_PREFIX="$OUTPUT_DIR/$LOCUST_RESULT"
    USE_OUT_DIR=false
  fi
  mkdir -p "$OUTPUT_DIR"
}

# locust가 생성한 _tmp_stats.csv → stats.csv 로 단순화 (LOCUST_OUT_DIR 모드 한정)
rename_outputs() {
  [ "${USE_OUT_DIR:-false}" = "true" ] || return 0
  local f
  for f in "$OUTPUT_DIR"/_tmp_*.csv; do
    [ -e "$f" ] || continue
    mv "$f" "$OUTPUT_DIR/$(basename "$f" | sed 's/^_tmp_//')"
  done
}

# meta.txt 자동 기록 (LOCUST_OUT_DIR 모드 한정)
write_meta() {
  [ "${USE_OUT_DIR:-false}" = "true" ] || return 0
  local commit phase
  commit=$(git -C "$SCRIPT_DIR" rev-parse --short HEAD 2>/dev/null || echo "unknown")
  phase="${LOCUST_PHASE:-$(infer_phase)}"
  cat > "$OUTPUT_DIR/meta.txt" <<EOF
phase: $phase
users: $LOCUST_USERS
spawn_rate: $LOCUST_RATE
duration: $LOCUST_TIME
mode: $LOCUST_MODE
host: $LOCUST_HOST
namespace: $K8S_NAMESPACE
commit: $commit
timestamp: $(date '+%Y-%m-%dT%H:%M:%S%z')
out_dir: $LOCUST_OUT_DIR
EOF
}

infer_phase() {
  case "$LOCUST_USERS" in
    20)   echo "smoke" ;;
    200)  echo "baseline" ;;
    500)  echo "peak" ;;
    1000) echo "stress" ;;
    *)    echo "custom_${LOCUST_USERS}u" ;;
  esac
}
...
do_save() {
  ...
  rename_outputs
  write_meta
  if [ "${USE_OUT_DIR:-false}" = "true" ]; then
    echo "=== 결과 저장: results/${LOCUST_OUT_DIR}/{stats,failures,exceptions,stats_history}.csv + meta.txt ==="
  else
...
do_spike() {
  ...
  write_meta
  if [ "${USE_OUT_DIR:-false}" = "true" ]; then
    echo "=== 결과 저장: results/${LOCUST_OUT_DIR}/ ==="
  else
    echo "=== 결과 저장: results/${LOCUST_RESULT}_*.csv ==="
  fi
}
...
do_verify() {
  ...
  echo "=== 데이터 정합성 검증 ==="
  if [ "${USE_OUT_DIR:-false}" = "true" ]; then
    run_psql -f - < "$SCRIPT_DIR/verify.sql" | tee "$OUTPUT_DIR/verify.txt"
    echo "=== 검증 결과: results/${LOCUST_OUT_DIR}/verify.txt ==="
  else
...
case "$CMD" in
  ...
  help|*)
    ...
    echo ""
    echo "결과 저장 옵션 (둘 중 하나):"
    echo "  LOCUST_OUT_DIR  새 폴더 구조 저장 (권장)"
    echo "                  예: LOCUST_OUT_DIR=round2/baseline_200u"
    echo "                  → results/round2/baseline_200u/{stats,failures,exceptions,stats_history}.csv"
    echo "                  → meta.txt 자동 생성 (phase, users, commit, timestamp 등)"
    echo "                  → verify 명령도 같은 폴더의 verify.txt에 저장"
    echo "  LOCUST_RESULT   legacy prefix 모드 (기본: k8s_<날짜시각>)"
    echo "                  → results/<prefix>_stats.csv 등 평탄 저장"
    echo ""
    echo "기타:"
    echo "  LOCUST_PHASE    meta.txt phase 표시 (미설정 시 USERS 기반 추정: 20=smoke, 200=baseline, 500=peak, 1000=stress)"
    echo ""
    echo "Spike 환경변수:"
